...
BLEUnsignedCharCharacteristic batteryLevelChar(
    ...
    BLERead | BLENotify); // remote clients will be able to get notifications if
...
BLEUnsignedCharCharacteristic secretValue("2a3F",
...
void setup() {
  ...
  pinMode(CTRL_LED, OUTPUT); // initialize the built-in LED pin to indicate when
  ...
  BLE.setGetIRKs([](uint8_t *nIRKs, uint8_t **BDaddrTypes, uint8_t ***BDAddrs,
    ...
    *BDaddrTypes = new uint8_t[*nIRKs];
    ...
    // Set these to the mac and IRK for your bonded devices as printed in the
    // serial console after bonding.
    uint8_t device1Mac[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t device1IRK[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    uint8_t device2Mac[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t device2IRK[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    (*BDaddrTypes)[0] =
        0; // Type 0 is for pubc address, type 1 is for static random
  ...
  BLE.setGetLTK([](uint8_t *address, uint8_t *LTK) {
    ...
    uint8_t device1Mac[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t device1LTK[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t device2Mac[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t device2LTK[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                              0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  while (1) {
    ...
    /* Set a local name for the BLE device
       ...
       The name can be changed but maybe be truncated based on space left in
    ...
    */
    ...
    batteryService.addCharacteristic(
        batteryLevelChar); // add the battery level characteristic
    ...
    batteryLevelChar.writeValue(
        oldBatteryLevel); // set initial value for this characteristic
    ...
    // prevent pairing until button is pressed (will show a pairing rejected
    // message)
...
void loop() {
  ...
  digitalWrite(PAIR_LED,
               (BLE.pairable() ? (millis() % 400) < 200 : BLE.paired())
                   ? PAIR_LED_ON
                   : !PAIR_LED_ON);
...
void updateBatteryLevel() {
  ...
  */
  ...
  if (batteryLevel != oldBatteryLevel) { // if the battery level has changed
    ...
    batteryLevelChar.writeValue(
        batteryLevel); // and update the battery level characteristic
